...
socket.on("triagem_call", function (data) {
  ...
  if (novaPrioridade.innerHTML == '1') {
    ...
    if (tableRow.find(function (a) {
      ...
      (0, _jquery["default"])('#tblPacientesTriagem > tbody > tr').eq(i - 1).after(novaLinha); // Adicionando após
    ...
    if (tableRow.find(function (a) {
      ...
      (0, _jquery["default"])('#tblPacientesTriagem > tbody > tr').eq(i).after(novaLinha); // Adicionando após
...
socket.on('triagemTela_call', function (data) {
  ...
  if (novaLinha.children[1].innerHTML == '1') {
    ...
    if (tableRow.find(function (a) {
      ...
      (0, _jquery["default"])('#tblPacientesTelaTriagem > tbody > tr').eq(i - 1).after(novaLinha); // Adicionando após
      // Se houver alguma linha com prioridade Muito Alta 
    } else if (tableRow.find(function (a) {
      return a.children[1].outerText == "Muito Alta";
    })) {
    ...
    if (tableRow.find(function (a) {
      ...
      (0, _jquery["default"])('#tblPacientesTelaTriagem > tbody > tr').eq(i).after(novaLinha); // Adicionando após
      // console.log('algum muito alta');
    } else {
